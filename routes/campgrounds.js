...
router.post("/", middleware.isLoggedIn, upload.single('image'), (req, res) => {
    ...
    geocoder.geocode(req.body.campground.location, (err, data) => {
        ...
        cloudinary.v2.uploader.upload(req.file.path, (err, result) => {
            if (err) {
                req.flash('error', err.message + ' (Error code: RT-CM-CRT)');
                return res.redirect('back');
            } else if (!result) {
    ...
    */
    ...
    Campground.findById(req.params.id, async (err, campground) => {
        if (err) {
            ...
            console.log('=============================v');
            console.log(campground.location ? 'campground.location = true' : 'campground.location = false');
            console.log(req.body.location.length ? 'req.body.location.length = true' : 'req.body.location.length = false');
            console.log('=============================^');
            if (campground.location !== req.body.location && !(!campground.location && !req.body.location)) {
                try {
                    await geocoder.geocode(req.body.location, (err, data) => {
                        console.log('pos 2');
                        // if (err) {
                        //     req.flash('error', err.message + ' (Error code: RT-CP-UP)');
                        //     return res.redirect('back');
                        // } else 
                        // if (!data) {
